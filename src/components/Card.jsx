...
function Card({
  ...
  deploiement,
  lienDemo,
  couleur = 'corail', // 'corail' | 'menthe' | 'jaune'
  tags = [],
}) {
  ...
  const handleTouchMove = (e) => {
    if (touchStartY.current !== null) {
      if (Math.abs(e.touches[0].clientY - touchStartY.current) > 8) didScroll.current = true
    }
  }
  ...
  return (
    <article className={`card card--${couleur}`}>
      {/* En-tête */}
      <div>
        <h3 className="card-title">{titre}</h3>
        {stackTechnique && (
          <p className={`card-tech tech--${couleur}`}>{stackTechnique}</p>
        )}
      </div>
      ...
      {hasDetails && (
        <div className={`card-details-wrap${detailsOpen ? ' card-details-wrap--open' : ''}`}>
          <div className="card-details-inner">
            ...
            {difficultes && (
              <div className="card-block">
                <p className="card-label">Difficultés surmontées</p>
      ...
      {/* Footer de carte : tags + liens */}
      <div className="card-footer">
        <div className="card-tags">
          {tags.map((tag) => (
            <span key={tag} className={`tag tag--${couleur}`}>{tag}</span>
          ))}
        </div>
        <div className="card-links">
          ...
          {lienDemo && (
            <a href={lienDemo} className="card-link card-link--demo" target="_blank" rel="noopener noreferrer">
              Voir le site →
            </a>
          )}
